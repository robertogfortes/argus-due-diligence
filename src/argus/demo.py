...
**Engagement:** {p.engagement_type} · **Jurisdiction:** {p.jurisdiction} · **Date:** {p.investigation_date}
...
**Overall Risk:** {p.overall_risk.value.upper()} · **Recommendation:** {p.recommendation.value.replace('_', ' ').upper()}

---

## Financial Risk Assessment — score {p.financial.score}/100 ({p.financial.risk_level.value.upper()})

{p.financial.summary}

{chr(10).join(f'- {x}' for x in p.financial.top_findings)}

## Reputational Risk Assessment — score {p.reputational.score}/100 ({p.reputational.risk_level.value.upper()})

{p.reputational.summary}

{chr(10).join(f'- {x}' for x in p.reputational.top_findings)}

## Legal & Compliance Assessment — score {p.legal.score}/100 ({p.legal.risk_level.value.upper()})

{p.legal.summary}

{chr(10).join(f'- {x}' for x in p.legal.top_findings)}

## Operational Assessment — score {p.operational.score}/100 ({p.operational.risk_level.value.upper()})

{p.operational.summary}

{chr(10).join(f'- {x}' for x in p.operational.top_findings)}
